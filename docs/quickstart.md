...
```bash
npm i -g @shieldnet360/secure-code-mcp
```
...
!!! note "Other channels"
    Run without installing: `npx -p @shieldnet360/secure-code-mcp secure-code-check <cmd>`. From source / contributors: `go install github.com/shieldnet-360/securevibe/cmd/skills-check@latest` (bare binary — point it at a data tree via `$SKILLS_LIBRARY_PATH`).
...
```json
{
  "mcpServers": {
    "secure-code": {
      "command": "npx",
      "args": ["-y", "@shieldnet360/secure-code-mcp"]
    }
  }
}
```
...
echo '{"jsonrpc":"2.0","id":1,"method":"tools/call",
  ...
  | npx -y @shieldnet360/secure-code-mcp
```
...
!!! note "List all available tools"
    ```bash
    echo '{"jsonrpc":"2.0","id":1,"method":"tools/list"}' | npx -y @shieldnet360/secure-code-mcp
    ```
...
- **Architecture** — read [ARCHITECTURE.md](https://github.com/shieldnet-360/securevibe/blob/main/ARCHITECTURE.md) for the full system design.
- **Signing model** — read [SIGNING.md](https://github.com/shieldnet-360/securevibe/blob/main/SIGNING.md) for key custody and rotation.
