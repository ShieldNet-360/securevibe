# Makefile for SecureVibe (github.com/shieldnet-360/securevibe)
#
# Thin wrappers around the commands documented in CONTRIBUTING.md so the
# build / test / validate flow is one `make <target>` away. The Go module
# path and source dirs stay github.com/shieldnet-360/securevibe and
# cmd/skills-check / cmd/skills-mcp (stable technical identifiers); the
# shipped CLI binary and user-facing command are `securevibe`.
...
LDFLAGS    ?= -s -w
BIN_DIR    ?= .
SECUREVIBE   := $(BIN_DIR)/securevibe

# Every .go file (excluding the VCS dir) is a prerequisite of the binary so
...
# produced phantom token-budget failures (the stale-./securevibe footgun
...
.PHONY: build
build: $(SECUREVIBE) ## Build the securevibe binary (default)
...
# canonicalises `./securevibe` to `securevibe`, so `make securevibe` still works as before — no separate phony alias
# needed (and a phony alias would create a circular self-dependency).
$(SECUREVIBE): $(GO_SOURCES)
	$(GO) build $(GOFLAGS) -ldflags "$(LDFLAGS)" -o $(SECUREVIBE) ./cmd/securevibe
...
.PHONY: validate
validate: $(SECUREVIBE) ## Validate every skill + rule file
	$(SECUREVIBE) dev validate

.PHONY: regenerate
regenerate: $(SECUREVIBE) ## Re-render dist/ (commit any drift)
