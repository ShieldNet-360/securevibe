...
// .deb and .rpm: name, contents listing a /usr/local/bin/securevibe
...
func TestNfpmConfigParses(t *testing.T) {
	...
	if cfg.Name != "securevibe" {
		t.Errorf("name=%q, want securevibe", cfg.Name)
	}
	if cfg.Description == "" {
	...
	for _, c := range cfg.Contents {
		if c.Dst == "/usr/local/bin/securevibe" {
			foundBinary = true
		}
	}
	if !foundBinary {
		t.Errorf("expected contents entry installing /usr/local/bin/securevibe; got %+v", cfg.Contents)
	}
}
...
func TestPackageManifestsPresent(t *testing.T) {
	...
	for _, rel := range []string{
		...
		"packaging/homebrew/securevibe.rb",
		"packaging/winget/shieldnet-360.securevibe.yaml",
		"packaging/scoop/securevibe.json",
...
func TestHomebrewFormulaShape(t *testing.T) {
	body, err := os.ReadFile(filepath.Join(repoRoot(t), "packaging", "homebrew", "securevibe.rb"))
	if err != nil {
		t.Fatal(err)
	}
	got := string(body)
