...
func sampleConfig() Config {
	return Config{
		Binary:   "/usr/local/bin/securevibe",
		Args:     []string{"update", "--regenerate", "--quiet"},
		Interval: 6 * time.Hour,
		LogPath:  "/tmp/securevibe-update.log",
	}
}
...
func TestRenderLaunchAgentPlistMatchesArchitectureDoc(t *testing.T) {
	...
	mustContain := []string{
		...
		`<string>/usr/local/bin/securevibe</string>`,
		`<string>update</string>`,
		`<string>--regenerate</string>`,
		`<string>--quiet</string>`,
		`<integer>21600</integer>`,
		`/tmp/securevibe-update.log`,
	}
...
func TestRenderSystemdServiceAndTimerMatchArchitectureDoc(t *testing.T) {
	...
	for _, want := range []string{
		...
		`Type=oneshot`,
		`ExecStart=/usr/local/bin/securevibe update --regenerate --quiet`,
	} {
...
func TestRenderTaskSchedulerXMLContainsRepetitionAndCommand(t *testing.T) {
	cfg := sampleConfig()
	cfg.Binary = `C:\Program Files\Skills-Check\securevibe.exe`
	...
	for _, want := range []string{
		...
		`<Command>C:\Program Files\Skills-Check\securevibe.exe</Command>`,
...
func TestDefaultsProducesUsableConfig(t *testing.T) {
	cfg := Defaults("/usr/bin/securevibe")
	if cfg.Binary == "" {
...
func TestSystemdPaths(t *testing.T) {
	if got := SystemdServicePath("/home/alice"); got != "/home/alice/.config/systemd/user/securevibe-update.service" {
		t.Errorf("service path: %s", got)
	}
	if got := SystemdTimerPath("/home/alice"); got != "/home/alice/.config/systemd/user/securevibe-update.timer" {
