...
import (
	...
	"github.com/shieldnet-360/securevibe/cmd/securevibe/internal/updater"
...
const releaseAssetOwnerRepo = "shieldnet-360/securevibe"
...
// `securevibe dev fetch-vulns --path .` is run from a checkout), then
...
func repoIngestScript(libraryRoot string) string {
	...
	if exe, err := os.Executable(); err == nil {
		...
		// "$repo/cmd/securevibe", and "$GOBIN" layouts.
		for i := 0; i < 4; i++ {
...
func fetchVulnsCmd() *cobra.Command {
	...
	c := &cobra.Command{
		...
		Long: `Populate the user-local OSV cache that skills-mcp (and securevibe
...
func runFetchVulnsFromRelease(c *cobra.Command, rawURL, cacheRoot string) error {
	...
	req.Header.Set("User-Agent", "securevibe/fetch-vulns")
