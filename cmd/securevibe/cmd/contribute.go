...
import (
	...
	"github.com/shieldnet-360/securevibe/cmd/securevibe/internal/manifest"
...
Sharing the block (the LEARN flywheel), in order of blast radius:
  ...
  • Your org    — point $SKILLS_CHECK_OVERLAY at a shared overlay file outside
    ...
    securevibe invocation folds it in, so an org-wide block applies across
...
func loadOverlay(path string) (*tools.OverlayFile, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			return &tools.OverlayFile{SchemaVersion: "1.0", GeneratedBy: "securevibe contribute"}, nil
		}
		return nil, err
	}
...
func contributeAddCmd() *cobra.Command {
  ...
  securevibe contribute add -p evil-pkg -e npm --reason "exfiltrates AWS creds in postinstall"

  # Block only specific versions, and sign the entry for provenance:
  securevibe contribute add -p left-pad -e npm --versions 1.0.0,1.1.0 --key ~/key.pem`,
		RunE: func(cmd *cobra.Command, args []string) error {
	...
	c := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(of.MaliciousPackages) == 0 {
				fmt.Fprintf(w, "No overlay rules yet (%s).\nAdd one with: securevibe contribute add -p <pkg> -e <eco> --reason <why>\n", path)
				return nil
			}
...
centrally-signed database.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			cand := candidateFile{
				...
				GeneratedBy:   "securevibe contribute submit",
