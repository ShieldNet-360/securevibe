...
func checksumLine(payload []byte) (binaryName, line string) {
	binaryName = fmt.Sprintf("securevibe-%s-%s", runtime.GOOS, runtime.GOARCH)
...
func TestSelfUpdateVerifiesSignature(t *testing.T) {
	...
	tmp := filepath.Join(t.TempDir(), "securevibe")
...
func TestSelfUpdateDetectsChecksumMismatch(t *testing.T) {
	...
	alter := func(name string) (string, bool) {
		if name == checksumName {
			return "deadbeef  securevibe-tampered\n", true
		}
		return "", false
	}
	srv, _, _ := newFakeReleaseServer(t, payload, alter)
...
func TestLookupChecksumIgnoresStarPrefix(t *testing.T) {
	r := strings.NewReader("abc123  *securevibe-linux-amd64\ndef456  securevibe-windows-amd64.exe\n")
	got, err := lookupChecksum(r, "securevibe-linux-amd64")
	if err != nil {
		t.Fatal(err)
	}
	if got != "abc123" {
