...
func TestSchedulerPreviewOutputs(t *testing.T) {
	stdout, _, err := executeRoot(t, "scheduler", "preview", "--target", "darwin", "--binary", "/usr/local/bin/securevibe")
	if err != nil {
		t.Fatal(err)
	}
	...
	stdout, _, err = executeRoot(t, "scheduler", "preview", "--target", "linux", "--binary", "/usr/local/bin/securevibe")
	if err != nil {
		t.Fatal(err)
	}
	...
	stdout, _, err = executeRoot(t, "scheduler", "preview", "--target", "windows", "--binary", "C:/securevibe.exe")
	if err != nil {
		t.Fatal(err)
	}
