...
// hosts the securevibe binaries and the matching per-target SHA-256
// checksum files.
const DefaultSelfUpdateBaseURL = "https://github.com/shieldnet-360/securevibe/releases/latest/download"
...
func selfUpdateCmd() *cobra.Command {
	...
	c := &cobra.Command{
		Use:   "self-update",
		Short: "Download the latest securevibe binary and atomically replace this one",
...
func runSelfUpdate(out io.Writer, baseURL, goos, goarch, targetPath string, dryRun, requireSig bool) (*selfUpdateResult, error) {
	binaryName := fmt.Sprintf("securevibe-%s-%s", goos, goarch)
	if goos == "windows" {
		binaryName += ".exe"
	}
