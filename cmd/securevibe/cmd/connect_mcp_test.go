...
func TestConnectMCPPrintDefault(t *testing.T) {
	out, err := runConnectMCP(t, "--print", "--binary", "/opt/securevibe", "--path", "/lib/root")
	if err != nil {
	...
	if !strings.Contains(out, "/opt/securevibe mcp --path /lib/root") {
		t.Errorf("default command should register `securevibe mcp` + library root; got:\n%s", out)
	}
	assertSnippetServer(t, out, defaultMCPName, "/opt/securevibe", []string{"mcp", "--path", "/lib/root"})
}
