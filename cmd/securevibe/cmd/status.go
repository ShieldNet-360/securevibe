...
run "securevibe update".`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if limit >= 0 && rep.VulnAgeDays > limit {
				return fmt.Errorf("vulnerability data is %d days old (limit %d); run `securevibe update`", rep.VulnAgeDays, limit)
			}
			return nil
		},
	}
...
func renderStatus(w interface{ Write([]byte) (int, error) }, rep statusReport) {
	...
	case "aging":
		fmt.Fprintf(w, "  freshness          ⚠️  aging — run `securevibe update` to refresh\n")
	case "stale":
		fmt.Fprintf(w, "  freshness          ❌ stale — your AI is reasoning about an outdated threat landscape; run `securevibe update`\n")
	default:
