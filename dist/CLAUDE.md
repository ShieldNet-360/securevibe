# CLAUDE.md — SecureVibe skills (minimal)

This file is generated by `securevibe dev regenerate`. Do not edit by
hand; update the source skills under `skills/` and regenerate.

Use the local security skills when generating or reviewing
security-sensitive code in this Claude Code project. Call the local skills MCP server
before finalizing.
...
- `llm-app-security` — LLM Application Security
- `logging-security` — Logging Security
- `ml-security` — ML Model Security
...
- For the full inlined skill bodies (the pre-v2 monolithic
  output), regenerate with `securevibe dev regenerate --full-inline`.
