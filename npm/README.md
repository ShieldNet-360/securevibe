...
over npm, so users can run it with `npx @shieldnet360/secure-code-mcp` — the
...
- **`@shieldnet360/secure-code-mcp`** — platform-agnostic. Ships `bin/launch.js`
...
- **`@shieldnet360/secure-code-mcp-<os>-<arch>`** ×5 — each carries only the
...
`require.resolve('@shieldnet360/secure-code-mcp-<key>/package.json')`, then
...
publishes all six packages at the release version. Requires the `@shieldnet-360`
